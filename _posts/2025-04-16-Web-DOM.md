...
## this
객체를 가르키는 키워드 ~~호출한 놈(이 없을 경우네는 window 객체)~~

‼️ **예외**
1. **전역스코프**에서 `this`는 window 객체
```js
...
2. **화살표 함수** : 자신이 포함하고 있는 외부 Scope에서 `this`를 계승받는다.
```js
let person1 = {
   name: "짐코딩",
   age: 20,
...
- **Strict Mode** : 엄격 모드에서는 기본값이 window가 아닌 `undefined`
```js
'use strict';
