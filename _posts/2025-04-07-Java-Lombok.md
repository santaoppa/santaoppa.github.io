...
- `@NonNull` : 메소드나 생성자의 매개변수에 사용하면 null check를 한다.
...
- `@RequiredArgsConstructor` : `final`, `@NonNull`이 있는 필드를 포함한 생성자를 생성한다.
...
- `@Data` : 모든 필드에 대해 `@ToString`, `@EqualsAndHashCode`, `@Getter`를, 모든 non-final 필드에 대해 `@Setter`를 설정하고 `@RequiredArgsConstructor`를 생성해주는 단축 Annotation
