---
title: "[React] 게시판 프로젝트 - (3) 데이터베이스 설계"
excerpt: "데이터베이스 설계 및 구현 "
...
React는 SPA(Single Page Application)으로 하나의 페이지를 지원하는 Application 이다.
따라서 Routing 기능을 지원하는 라이브러리를 설치해 사용해야함

A태그는 화면을 새로고침한 다음에 페이지를 이동하지만, 
React Router에서는 새로운 페이지를 로드하지 않고 하나의 페이지 안에서 필요한 데이터만 가져오기 때문에 불필요한 렌더링을 막을 수 있다.
